...
/**
 * <p>
 * A HistogramTable is a subclass of (@link prefuse.data.Table), but that has
 * been histogramized: one column of the original (@link prefuse.data.Table)
 * gets counted and slotted into a Table. The first (@link
 * prefuse.data.column.Column) is the ranges of the data, with the value of the
 * cell corresponding to the minimum value of the range. The second column holds
 * the number of each element in the original Table that falls within the data
 * range represented by the cell in column 1.
 * 
 * NOTE: This only works with numeric and string fields. It has not been tested
 * with booleans or derived fields. Booleans will probably be treated as
 * strings.
 * 
...
public class HistogramTable extends TableEx {
    ...
    // m_bin{Min, Max} are the min and max of the data column BUT for Strings, min
    // is 1
    ...
    /**
     * @param aTable    a Prefuse Table with data values (i.e. non-histogrammized)
     *                  in it
	...
	for (int fieldIndex = 0; fieldIndex < fieldNames.length; fieldIndex++) {
		...
		// Can't really histogrammize any other data in a sensible way, ignore it
		// *shrug*
		continue;
	    }
	}
     ...
     * @return fieldNames a list of the names of the columns in the table. Note that
     *         a HistogramTable will have all the same column names as are in its
     *         (Prefuse Table) data table's, but will also have an additional set of
     *         columns that have the counts. See getCountField(). TODO It might be
     *         interesting to have a method getNonCountFieldNames which strips out
     *         the count fields.
     */
     ...
     * @param dataColumn the dataColumn to histogrammize Note: I was unable to
     *                   figure out how to get the name of a column from a Column
     *                   object. That seemed odd. -- KDS Note that I don't actually
     *                   have to pass in the dataColumn -- I can get that from the
     *                   field name -- but the dataColumn happens to be handy, so
     *                   why not.
     */
    ...
    /**
     * @param field      the name of the dataColumn to histogrammize
     * @param dataColumn the dataColumn to histogrammize
     */
	...
	m_columns = new ArrayList<Object>(columnCount);
	m_names = new ArrayList<Object>(columnCount);
	m_rows = new RowManager(this);
	m_entries = new HashMap<Object, Object>(columnCount + 5);
    ...
    /**
     * Initialize the bin column. The bin columns have information on the range of
     * values that the count columns have counts for. For example, you can say
     * "there are 17 elements between the value of 2 and 14". 17 would be the value
     * in the count field, and 2-14 would be represented by the bin field. Note that
     * the way that bin fields are represented, the value in the bin field is the
     * low end of the range. In the example, the bin field would have a 2 in it.
     * 
     * @param field the name of the dataColumn to histogrammize
     */
     ...
     * 
     * @param field      the name of the dataColumn to histogrammize
     * @param dataColumn the column in the original (@link prefuse.data.Table) to be
     *                   histogramized.
     */
     ...
     * @return the minimum and maximum values of the associated count column in an
     *         array.
     */
    ...
    /**
     * @param field the name of the histogramColumn
     * @return the minimum value of the associated count column (In other words, if
     *         you ask for getCountMin("A"), you will get the min of the column "A
     *         counts".)
     */
    ...
    /**
     * @param field the name of the histogramColumn
     * @return the max value of the associated count column (In other words, if you
     *         ask for getCountMin("A"), you will get the max of the column "A
     *         counts".)
     */
