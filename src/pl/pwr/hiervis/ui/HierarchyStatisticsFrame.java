...
public class HierarchyStatisticsFrame extends JFrame {
	...
	cboxSubtree.setToolTipText(SwingUIUtils
		.toHTML("If checked, measures will be computed for the currently selected node and its subtree.\n"
			+ "Otherwise, measures will be computed for the currently selected node only."));
	cboxSubtree.addItemListener(e -> {
	...
	mntmDump.addActionListener((e) -> {
		...
		context.getMeasureManager().dumpMeasures(Paths.get(fileDialog.getSelectedFile().getAbsolutePath()),
	...
	Collection<MeasureTask> validMeasureTasks = measureManager
		.getMeasureTasks(task -> task.applicabilityFunction.apply(h));
	...
	for (String groupPath : measureManager.listMeasureTaskGroups()) {
	    Collection<MeasureTask> measureTasks = measureManager.getMeasureTaskGroup(groupPath).stream().sorted()
		...
		addPanels(mainPanel, createFillerPanel(10), createSeparatorPanel(friendlyGroupName));
	...
	if (context.getHierarchy().measureHolder.isMeasureComputed(h, task)) {
	    cMeasure.add(createMeasureContent(task, context.getHierarchy().measureHolder.getMeasureResult(h, task)),
		    BorderLayout.NORTH);
	}
	else {
	...
	button.addActionListener((e) -> {
		...
		if (!lh.measureHolder.isMeasureComputed(h, task) && !measureManager.isMeasurePending(h, task)) {
	...
	button.addActionListener((e) -> {
		...
		measureManager.postTask(context.getHierarchy().measureHolder, task.getLeft(), task.getRight());
	    }
	});
    ...
    /**
     * Creates a GUI component used to represent the specified measure computation
     * result.
     * 
     * @param result the measure computation result to create the component for
	...
	if (result instanceof double[]) {
		...
		buf.append(Integer.toString(i)).append(":\t").append(formatDoubleValue(data[i]));
	    ...
	    buf.append(formatDoubleValue(avg.getAvg())).append(" ± ").append(formatDoubleValue(avg.getStdev()));

	    if (measure.isQualityMeasure()) {
		buf.insert(0, "Value: ").append('\n').append("Desired:\t")
			.append(formatDoubleValue(measure.getDesiredValue())).append('\n').append("Undesired:\t")
			.append(formatDoubleValue(measure.getNotDesiredValue()));
	    }
	    ...
	    throw new IllegalArgumentException(
		    String.format("No case defined for data type '%s'", result.getClass().getSimpleName()));
	}
    }
	...
	updateMeasurePanel(result.getKey().getLeft(), result.getKey().getRight(), result.getValue());
    }
	...
	SwingUtilities.invokeLater(() -> {
	    ...
	    // This code is deferred and executed on the main thread, so there's no
	    // guarantee that
	    // it will actually get to run before the measure is computed.
	    // If the measure was computed before we got here, then there's nothing for us
	    // to do.
	...
	if (context.getHierarchy().isOwnerOf(result.getLeft())) {
	    SwingUtilities
		    .invokeLater(() -> updateMeasurePanel(result.getLeft(), result.getMiddle(), result.getRight()));
	}
    }
