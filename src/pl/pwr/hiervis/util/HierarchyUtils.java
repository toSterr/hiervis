...
public class HierarchyUtils {
	 ...
	 * 
	 * @param source
	 *            the source hierarchy that is to be merged into destination
	 *            hierarchy
	 * @param dest
	 *            the destination hierarchy that will receive nodes from source
	 *            hierarchy
	 * @param nodeId
	 *            id of the node in the destination hierarchy that will serve as the
	 *            merging point for the two hierarchies. Nodes from the source
	 *            hierarchy will be rebased to have this id as root.
	...
	public static LoadedHierarchy merge(LoadedHierarchy source, LoadedHierarchy dest, String nodeId) {
		...
		return new LoadedHierarchy(merge(source.getMainHierarchy(), dest.getMainHierarchy(), nodeId), source.options);
	}
	...
	public static LoadedHierarchy flattenHierarchyKeepReductons(LoadedHierarchy source) {
		...
		Hierarchy mainHierarchy = flattenHierarchy(source.getHierarchyWraper().getOriginalHierarchy());
		...
		for (int i = 0; i < reducedHierarchy.length; i++) {
			if (source.getHierarchyWraper().getReducedHierarchy(i) != null)
				reducedHierarchy[i] = flattenHierarchy(source.getHierarchyWraper().getReducedHierarchy(i));
			else
	...
	/**
	 * Creates a new Hierarchy (a deep copy) that doesn't contain the node with the
	 * specified id.
	 * 
	 * @param source
	 ...
	 * is a deep copy of the specified node (and its subtree, if that option is
	 * selected)
	 * 
	 * @param source
	...
	public static Hierarchy wrapNode(Hierarchy source, Node node, boolean withSubtree) {
		...
		if (withSubtree) {
			Arrays.stream(source.getGroups()).filter(n -> n.getId().startsWith(node.getId()))
					.forEach(n -> nodes.add(clone(n, false)));
		}
		else {
			Arrays.stream(source.getGroups()).filter(n -> n.getId().equals(node.getId()))
					.forEach(n -> nodes.add(clone(n, false)));
		}
	...
	/**
	 * Checks whether the specified hierarchy contains the specified node (by
	 * reference).
	 * 
	 * @param h
	...
	/**
	 * Creates a sub-hierarchy of the specified {@link Hierarchy}, which contains
	 * the specified node as root, and all its child nodes. The newly created
	 * hierarchy is a deep copy of the source hierarchy.
	 * 
	 * @param source
	 *            the hierarchy to create the sub-hierarchy from
	 * @param nodeId
	 *            id of the node within the source hierarchy, which will be the root
	 *            in the sub-hierarchy. Defaults to {@link Constants#ROOT_ID} if
	 *            null.
	 * @param destNodeId
	 *            the new id that the copied nodes will receive. Defaults to
	 *            {@link Constants#ROOT_ID} if null.
	...
	/**
	 * Rebases all nodes and instances in the specified stream from the specified
	 * old id to the new id. This method creates copies of all objects (nodes,
	 * instances) with the same properties, but changed ids.
	 * 
	 * The rebased nodes have no parent-child relationships set, requiring a
	 * follow-up call to a method that will build them.
	 * 
	 * @param nodes
	 *            stream of nodes to rebase
	 * @param oldId
	 *            the old id to rebase. Only nodes that contain this substring will
	 *            be rebased.
	 * @param newId
	 ...
	 * @param useSubtree
	 *            whether the centroid calculation should also include child nodes'
	 *            instances
	 */
	public static void rebase(Stream<Node> nodes, String oldId, String newId, List<BasicNode> destNodes,
			boolean useSubtree) {
		...
		nodes.filter(n -> n.getId().equals(oldId) || HierarchyBuilder.areIdsAncestorAndDescendant(oldId, n.getId()))
				.forEach(n -> {
					...
					n.getNodeInstances()
							.forEach(in -> rebasedNode.addInstance(new BasicInstance(in.getInstanceName(), id,
	...
	/**
	 * Creates a deep copy of the specified hierarchy, creating copies of
	 * {@link Node}s and {@link Instance}s that comprise the {@link Hierarchy}.
	 * 
	 * One exception is that {@link Instance}s are not cloned 100% deeply -- the
	 * feature values array is copied by reference, since it is never modified, at
	 * least for now.
	 * 
	 * @param source
	 ...
	 *            predicate allowing to select nodes that should be included in the
	 *            cloned hierarchy. Can be null to include all nodes.
	...
	public static Hierarchy clone(Hierarchy source, boolean useSubtree, Predicate<Node> nodeInclusionPredicate) {
		...
		Arrays.stream(source.getGroups()).forEach(n -> {
	...
	/**
	 * Creates a deep copy of the specified node, creating copies of
	 * {@link Instance}s that have been assigned to this {@link Node}.
	 * 
	 * @param source
	 *            the node to clone
	 * @param useSubtree
	...
	public static Instance clone(Instance source) {
		return new BasicInstance(source.getInstanceName(), source.getNodeId(),
				// NOTE: Technically we should copy the array for a true deep copy,
				// however in the context of this application it makes no sense, since
				// we never modify feature values of instances, and duplication of these
				// arrays ends up being a considerable memory hog.
				// Arrays.copyOf( source.getData(), source.getData().length ),
				source.getData(), source.getTrueClass());
	}
	...
	/**
	 * Serializes the specified hierarchy into CSV format with the specified
	 * settings, and saves it in the specified file with UTF-8 encoding.
	 * 
	 * @param path
	 ...
	 * @param withHeader
	 *            whether the serialized CSV should include a header with column
	 *            names
	 * @throws IOException
	 ...
	 */
	public static String toCSV(Hierarchy h, boolean withAssignClass, boolean withTrueClass, boolean withInstanceNames,
			boolean withHeader) {
		int capacity = 0;
		try {
	...
	/**
	 * Attempts to estimate the length of string required to serialize the specified
	 * hierarchy, given the specified options. If the required capacity overflows
	 * range of 32-bit integer, this method throws an {@link ArithmeticException}.
	 * 
	 * @param h
	 *            the hierarchy to serialize
	 * @param withAssignClass
	 ...
	 */
	private static int estimateBufferSize(Hierarchy h, boolean withAssignClass, boolean withTrueClass,
			boolean withInstanceNames) {
		...
		if (withAssignClass)
			result = Math.addExact(result, charsForNodeId);
		if (withTrueClass)
			result = Math.addExact(result, charsForNodeId);
		if (withInstanceNames)
			result = Math.addExact(result, instances * charsForInstanceName);

		return result;
	}

	/**
	 * Constructs a CSV file header for the specified hierarchy and settings, and
	 * appends it to the buffer without a newline at the end.
	 */
	private static void appendCSVHeader(StringBuilder buf, Hierarchy h, boolean withAssignClass, boolean withTrueClass,
			boolean withInstanceNames) {
		if (withAssignClass)
			buf.append("class;");
		if (withTrueClass)
			buf.append("true_class;");
		if (withInstanceNames)
	...
	/**
	 * Constructs a CSV file row for the specified instance and settings, and
	 * appends it to the buffer without a newline at the end.
	 */
	private static void appendInstance(StringBuilder buf, Instance instance, boolean withAssignClass,
			boolean withTrueClass, boolean withInstanceNames) {
		if (withAssignClass)
			buf.append(instance.getNodeId()).append(';');
		if (withTrueClass)
			buf.append(instance.getTrueClass()).append(';');
		if (withInstanceNames)
