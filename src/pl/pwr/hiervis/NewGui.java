...
import pl.pwr.hiervis.ui.jakubTestuje.DockerUI;
import pl.pwr.hiervis.util.SwingUIUtils;

public final class NewGui {
	...
	if (new File(".").exists() == false) {
	    ...
	    SwingUIUtils.showErrorDialog("Program was unable to access the current working directory.\n\n"
	...
	if (cmd.hasOption('i')) {
	    ...
	    loadOptions = new LoadedHierarchy.Options(withInstanceNames, withTrueClass, withHeader, fixBreadthGaps,
		    useSubtree);
    ...
    private static void executeGUI(HVContext ctx, String subtitle, File inputFile,
	    LoadedHierarchy.Options loadOptions) {
	...
	if (SystemUtils.IS_OS_UNIX && SwingUIUtils.isXFCE() && SwingUIUtils.isOpenJDK()) {
	    // Unix systems running XFCE desktop environment with OpenJDK experience a
	    // complete freeze
		...
		if (config.isStopXfceLafChange()) {
			...
			prefLAF = Arrays.stream(UIManager.getInstalledLookAndFeels())
				.filter(laf -> !laf.getName().toLowerCase(Locale.ENGLISH).equals("metal")).findFirst()
				.get().getName();
	...
	if (prefLAF != null && !prefLAF.isEmpty()) {
		...
		LookAndFeelInfo lafInfo = Arrays.stream(UIManager.getInstalledLookAndFeels())
			.filter(laf -> laf.getName().equals(fprefLAF)).findFirst().get();

		UIManager.setLookAndFeel(lafInfo.getClassName());
		successLAF = true;
	    }
	    catch (NoSuchElementException | ClassNotFoundException | UnsupportedLookAndFeelException e) {
		log.printf(Level.ERROR, "Could not find a matching L&F for name '%s'. Falling back to system default.",
			prefLAF);
	    }
	    catch (Exception ex) {
	    ...
	    if (!successLAF) {
		log.printf(Level.INFO, "Could not find a matching L&F for name '%s'. Falling back to system default.",
			prefLAF);
	    }
	}
	...
	if (!successLAF) {
	    ...
	    // Set the preferred L&F to the current one, so that we don't have to look it up
	    // again,
	...
	// Windows L&F uses Monospaced 9 as default font for JTextAreas, but other L&Fs
	// don't.
	// Fix that by manually setting the default to the same font as JTextField.
	UIManager.getDefaults().put("TextArea.font", UIManager.getFont("TextField.font"));

	try {
	    // Due to some internal workings of Swing, default sounds are not played for
	    // non-default L&Fs.
	...
	// Attempt to set the application name so that it displays correctly on all
	// platforms.
	...
	// ostatnia rzecz jaka musi sie wykonac
	EventQueue.invokeLater(() -> startup(ctx));
    }

    @SuppressWarnings("static-access")
    private static void createOptions() {
	Option inputOpt = OptionBuilder.withArgName("file path").hasOptionalArgs(4).hasArgs(5).isRequired(false)
		.withDescription("path to a *.csv file describing a hierarchy to load on app start").create("i");

	Option subtitleOpt = OptionBuilder.withArgName("name").hasArgs(1).isRequired(false)
		.withDescription("optional subtitle for frame titles, to help identify them").create("s");

	Option helpOpt = OptionBuilder.hasArg(false).isRequired(false).withDescription("prints this message")
	...
	PatternLayout layout = PatternLayout.createLayout(
		"%d{HH:mm:ss.SSS} [%t] %-5level %logger{36} - %msg%throwable%n", null, null, null,
		Charset.defaultCharset(), false, false, null, null);

	FileAppender appender = FileAppender.createAppender("logs/log-" + subtitle + ".txt", "false", "false",
		"LogFile-" + subtitle, "true", "true", "true", "8192", layout, null, "false", "", config);
